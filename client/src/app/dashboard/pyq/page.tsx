...
export default function PYQArchive() {
    ...
    if (loading) return <div className="p-10 sm:p-20 text-center font-bold text-slate-400">Loading Archive...</div>;

    const items = currentLevel();

    return (
        <div className="min-h-screen bg-slate-50 p-4 sm:p-6 md:p-12 space-y-8">
            <div className="max-w-5xl mx-auto space-y-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight">PYQ ARCHIVE</h1>
                ...
                {path.length < 3 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {path.length > 0 && (
                            <div 
                                onClick={goBack}
                                className="p-5 sm:p-8 bg-white rounded-3xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-300 hover:bg-indigo-50/30 transition-all group"
                            >
                        ...
                        {items && items.map((item: string) => (
                            <div 
                                ...
                                className="p-5 sm:p-8 bg-white rounded-3xl border-2 border-slate-100 shadow-sm hover:shadow-xl hover:border-indigo-400 hover:scale-[1.02] transition-all cursor-pointer group relative overflow-hidden"
                            >
                                ...
                                <h3 className="text-xl sm:text-2xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors uppercase tracking-tight break-words">{item}</h3>
                    ...
                    <div className="space-y-6">
                        <div className="bg-indigo-600 p-5 sm:p-10 rounded-3xl sm:rounded-[2.5rem] shadow-2xl shadow-indigo-200 text-white relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-32 -mt-32 blur-3xl" />
                            <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:justify-between lg:items-end">
                                <div className="space-y-2">
                                    ...
                                    <h2 className="text-3xl sm:text-4xl font-black tracking-tight uppercase break-words">{path[1]}</h2>
                                    <p className="text-indigo-100 font-medium">Practice {items?.length || 0} essential questions from this session.</p>
                                </div>
                                <Button className="bg-white text-indigo-600 hover:bg-indigo-50 font-black px-6 sm:px-8 py-6 rounded-2xl shadow-xl flex gap-2 items-center text-base sm:text-lg w-full sm:w-auto">
                        ...
                        <div className="grid gap-4">
                            {items && items.map((q: any, idx: number) => (
                                <div key={q.id} className="bg-white p-5 sm:p-8 rounded-3xl border border-slate-100 shadow-sm transition-all hover:shadow-md">
                                    ...
                                    <p className="text-lg sm:text-xl font-bold text-slate-800 leading-relaxed mb-6 break-words">{q.text}</p>
